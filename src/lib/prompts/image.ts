...
- Color palette: 3-5 dominant colors, 1-2 accents. Saturation level: desaturated / muted / natural / vivid. Default to "natural" or "muted" for real-world photographs — most phone and camera photos do not produce vivid saturation without HDR processing or intentional grading. "Vivid" should only be used when colors are demonstrably more intense than real-life perception, such as in HDR-overprocessed images, heavily graded editorial work, or neon/artificial color environments. Note palette separation, color blocking, and whether the palette feels unified, split, pastel, neon, earthy, sterile, or dirty. Distinguish neutral silver/grey from cool cyan-blue steel carefully; do not exaggerate a mild cool cast into a strong stylized metallic blue grade. Saturation realism: if the colors look like what a human eye would see in that real environment, the saturation is "natural" — not "muted" and not "vivid."
...
- Contrast shaping: whether the image relies on deep blacks, lifted shadows, muted highs, luminous mids, or isolated bright peaks. **Default to moderate contrast for all real-world photographs.** The vast majority of phone photos, ambient-lit interiors, overcast outdoor shots, window-lit portraits, and casual snapshots have moderate tonal separation — not crushed blacks, not blown highlights, not dramatic contrast. Only describe high contrast (deep crushed shadows + bright clipped highlights) when the image shows unmistakable evidence: near-black shadow regions with zero detail, pure white highlight patches with no texture, or extreme exposure splits from direct flash at night or harsh midday sun with deep shade. If shadow detail and highlight detail are both visible, the contrast is moderate or low-moderate — describe it that way. Do not use "high contrast", "dramatic contrast", "punchy", or "deep blacks" to describe ordinary lighting.
...
- Shot type: close-up (face only) / head-and-shoulders / bust (head to mid-chest) / half-body (head to waist) / three-quarter (head to knees) / full body / wide (subject small within environment). State the top and bottom crop boundaries on the body explicitly using anatomical landmarks (e.g., "top: crown of head, bottom: mid-sternum" for bust shot). This is a locked parameter — the output image MUST show exactly the same body range.
- Camera angle: eye-level / low angle / high angle / Dutch tilt. State approximate degrees.
...
- Subject scale in frame: **LOCKED PARAMETER.** State the exact percentage of frame height the subject occupies (e.g., "subject occupies ~65% of frame height from crown to visible bottom"). Describe exactly where the bottom crop falls on the body (e.g., "bottom crop at mid-chest / waist / hip / knee"). State whether the image feels subject-dominant, balanced with environment, or environment-dominant. The generator MUST NOT zoom in or make the subject fill more of the frame than the original.

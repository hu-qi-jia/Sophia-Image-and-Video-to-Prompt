...
(7) BEAUTY PROCESSING — present? If yes, name the specific style (Douyin/Instagram/Korean ulzzang/小红书/Weibo)
...
- Beauty-filtered: "This is a polished Douyin-style beauty portrait photograph. The image has a warm golden-hour glow with soft diffused ambient light, 3500K warmth, and lifted shadow tones with subtle peachy-pink undertones. Skin shows AI smoothing with porcelain-like uniformity and subtle face-slimming jawline geometry. The medium texture is clean digital smartphone capture with a soft radial vignette and subtle beauty-filter bloom on highlights."
...
- **Color distribution**: where solid fields, where gradients, where high saturation sits in the frame.
...
- **Beauty/portrait processing** (if applicable): AI skin smoothing level (subtle/moderate/heavy — porcelain-like), face reshaping geometry, skin tone evening, under-eye brightening, lip/eye enhancement. Note whether natural-subtle or heavy-obvious.
...
- **Beauty & social media aesthetic detection** (CRITICAL for portraits with visible post-processing — this IS the style, preserve it):
  - Douyin/抖音: heavy AI skin smoothing (porcelain-like), subtle jawline narrowing + chin sharpening, enlarged eyes with bright catchlights, warm golden glow, soft radial vignette
  - Instagram/TikTok: moderate skin smoothing, boosted vibrance, lifted shadows, warm tone shift, subtle highlight bloom
  - Korean ulzzang: glass-skin effect (extremely smooth luminous skin, no visible pores), soft pastel color grading, subtle aegyo-sal (under-eye highlight)
  ...
  - Western influencer: warm skin tones, slight desaturation in shadows, clarity/texture boost, subtle orange-teal split toning
  If beauty processing is detected, name the specific style AND describe its visual characteristics. Do NOT dismiss as "just a filter."
- **Art style** (for non-photographic images — anime/illustration/painting): line quality (clean vector / hand-drawn wobble / rough sketch / variable weight), shading method (cel-shading 2-3 tone / soft gradient / crosshatching / screentone / painterly), rendering technique, medium simulation (watercolor paper bleed, ink spread, paint impasto, digital cleanliness).
...
- **Cross-material interaction**: color bleeding between adjacent surfaces, halation contamination, bloom reflecting off nearby objects. Include frame positions.
...
// ── DIAGNOSTIC STYLE TAGs (always required) ───────────────────────────
...
Quality boosters (select 2-4, MODE-AWARE):
...
- **Mode B — Source HAS beauty processing** (beauty-filtered/social-media/glamour): "preserve the beauty-filtered skin smoothing, maintain face-slimming proportions, keep enlarged bright eye catchlights, preserve warm skin glow, do not add raw skin texture or pores that the source does not show, do not de-beautify — the polished appearance IS the source style."
...
- Output modules in this exact order: [ARCHETYPE] → [AESTHETIC HOOK] → [COLOR] → [LIGHTING] → [STYLE & TEXTURE] → [FRAME] → [COMPOSITION] → [ATMOSPHERE] → [MATERIAL RESPONSE] → [ERA SIGNALS] → [PROMPT TAGS] → [NEGATIVE PROMPT] → [SUBJECT 1..N] → [SPATIAL LAYERS] → [ENVIRONMENT] → [IMPERFECTIONS & PHYSICS] → [CONSTRAINTS].`;
}

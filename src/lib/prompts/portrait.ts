...
6. **Qualified Direction (CANONICAL).** Every direction word must declare its reference frame. Default to viewer-relative direction: viewer-left, viewer-right, upper-left, upper-right, lower-left, lower-right, center. Never write bare "left" or "right" anywhere in the output, including tags, clothing, body parts, objects, negatives, or constraints. Only use subject-left / subject-right when explicitly labeling the subject's own body side. Prefer stable anchors when possible: "toward the window mullion", "table-side hand", "window-side shoulder", "camera-near hip".
...
//          (5) Centerline — does the camera sit on the subject's vertical centerline, or is it offset viewer-left / viewer-right? State the offset.
...
//      7. Direction rule (canonical): for gaze and head turn, use viewer-relative wording, subject-left / subject-right when explicitly body-referenced, or explicit object targets; never bare left/right.
//
...
//      6. Spine curve
//      7. Shoulder heights
//      8. Limb positions with explicit viewer-relative, subject-relative, or anchor-relative direction
//      9. Hand placement
...
//    foreground / midground / background elements with frame coverage, occlusion chain, and subject layer relative to anchors. For major anchors, use a 4-axis position tuple: frame quadrant, depth layer, viewer-relative clock direction, subject-relative direction. Never use bare "left" or "right". For non-studio portraits with distinctive environments, list 5-10 anchors (more for deep multi-layer scenes; fewer only for shallow or studio backdrops). State subject contact with furniture / surface explicitly, plus subject depth position and nearest object in front of and behind the subject. Ground truth over plausible-scene inference: do not invent "on bed", "at desk", "behind counter", etc. unless visible. Preserve front-to-back occlusion order.
//    I.1 Anchor-coordinate lock (mandatory for non-studio portraits with distinctive environments). Give approximate frame coverage and quadrant coordinates for the 3-6 strongest anchors, then state how the subject overlaps or sits beside each one. Preserve the anchor triangle/quadrilateral relationship: do not relocate, scale up/down, swap sides, center, simplify, or replace major anchors. If an anchor is cropped by the frame edge, state the crop; cropped anchors must remain cropped.
//
// J. Environment (required unless pure studio backdrops; ref: §CORE RULE 6 for direction):
//    sky, ground / surface, weather, indoor/outdoor, background fixtures / structures, time of day and season cues. Every named fixture / structure / furniture anchor should carry the same 4-axis position tuple, plus approximate frame coverage when spatially important. When the source is an indoor residential / hospitality / commercial space, describe the visible wall, ceiling, floor treatments, the window(s) and what is seen through them, any large mounted electronics and their state, visible furniture, and any decorative elements that occupy meaningful frame area — naming only what is actually visible, not a generic interior template.
//    J.1 No environment restaging. For themed interiors, preserve the actual visible prop layout rather than generating a plausible alternate set. Posters, lamps, ropes, windows, chairs, table edges, plates, utensils, and wall panels must keep their observed side, depth, scale, and crop relationships to the subject.
//
...
- For fitted glamour silhouettes, include source-specific pose/body negatives when they oppose the source: square shoulders, straight torso, frontal average stance, flattened S-curve, reduced hip shelf, covered thigh slit, lowered slit, boxy dress fit, hidden waist pinch, missing side-body contour, missing support hand, missing desk contact. Qualify any directional negative with viewer-/subject-/anchor-relative wording; never write bare left/right.
...
- Direction self-check: before finalizing, replace every bare "left" or "right" with viewer-left / viewer-right / subject-left / subject-right / upper-left / upper-right / lower-left / lower-right / object-anchored wording. This applies to all modules, including NEGATIVE PROMPT and CONSTRAINTS.

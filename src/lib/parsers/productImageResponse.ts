...
// Required TAGs that must be present in a valid product image response
// (40% Style module: 6 core + 3 diagnostic)
const REQUIRED_TAGS = [
  "ARCHETYPE",
  "AESTHETIC HOOK",
  "LIGHTING",
...
// Content TAGs that must be present (PRODUCT is matched by prefix)
...
function validateSections(sections: Record<string, string>): string[] {
  ...
  // Check PRODUCT content length specifically
  const productKey = Object.keys(sections).find(k => k.startsWith("PRODUCT"));
  if (productKey && sections[productKey].length < MIN_CONTENT_LENGTH) {
    warnings.push(`[${productKey}] 内容过短（${sections[productKey].length} 字符）`);
  }
...
function extractShortPromptFromSections(sections: Record<string, string>): string {
  const productKey = Object.keys(sections).find(k => k.startsWith("PRODUCT"));
  const productContent = productKey ? sections[productKey] : "";
  const productLabel = productContent.split(/\n/)[0]?.trim() || "";
  const framework = sections["FRAME"] || "";
  const firstSentence = framework.split(/[.。]/)[0] || "";
  return `${firstSentence} ${productLabel}`.trim() || "Product image analysis";
}
...
const STYLE_TAGS = new Set([
  "ARCHETYPE", "AESTHETIC HOOK", "LIGHTING",
  "OPTICAL DEPTH", "STYLE & TEXTURE", "FRAME",
  "GENERATION CUES", "PROMPT TAGS", "NEGATIVE PROMPT"
]);

const CONTENT_TAG_PREFIXES = ["PRODUCT", "PRODUCT DETAILS", "SET DRESSING", "CONSTRAINTS"];
...
export function parseProductImageResponse(rawText: string): ImagePromptResponse {
  const sections = parseSectionedText(rawText);

  if (Object.keys(sections).length === 0) {
    throw new Error("无法解析产品图片分析结果，请重试。(E6)");
  }
...
function normalizeStructuredImageResponse(
  ...
  response.recreation_anchors = {
    must_preserve: mustPreserve.length > 0 ? mustPreserve : ["product identity", "viewpoint", "crop"],
    high_risk_errors: highRiskErrors.length > 0 ? highRiskErrors : ["wrong product", "wrong crop"],
    priority_order: safeTrim(anchors?.priority_order) || "product identity and composition matter most",
  ...
  if (
    ...
    throw new Error("模型返回了无效的响应格式，请重试。(E7)");
  }
...
function normalizeLegacyImageResponse(
  ...
  if (
    ...
    throw new Error("模型返回了无效的响应格式，请重试。(E8)");
  }
...
export function parseGeminiProductImageResponse(rawText: string): {
  imageSummary: string;
  ...
  if (rawText.includes("[") && rawText.includes("]")) {
    try {
      const result = parseProductImageResponse(rawText);
      return {
...
export function formatProductImagePrompt(
...
export function formatProductImageAnalysis(
  promptResult: ImagePromptResponse | GeminiImagePromptResponse | StructuredImagePromptResponse
): string {
  if ("sections" in promptResult) {
